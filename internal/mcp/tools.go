	...
	return []ToolDefinition{
		{
			...
			InputSchema: obj(
				prop("semantic_session_id", "string", "Optional semantic session id from session-mcp. Preferred over session_id."),
			...
			InputSchema: obj(
				...
				propObject(
					...
					obj(
						...
						prop("script_result", "string", "Script output summary (required)."),
						...
						propArrayOfObject(
							...
							obj(
								...
								prop("output", "string", "Command output summary."),
			...
			InputSchema: obj(
				...
				propObject(
					...
					obj(
						...
						prop("script_result", "string", "Acceptor script output summary (required)."),
		...
		{
			Name:        "setWorker",
			Description: "Set the active worker ID for this connection.",
			InputSchema: obj(
				prop("semantic_session_id", "string", "Optional semantic session id from session-mcp. Preferred over session_id."),
				prop("session_id", "string", "Optional session id (cookie-like)."),
				prop("worker_id", "string", "Worker ID (optional; defaults to current member_id)"),
...
func allToolsForRole(role string) []ToolDefinition {
	...
	if allowed == nil {
		...
		return injectPassportTokenIntoTools(role, base)
	}
	...
	return injectPassportTokenIntoTools(role, out)
}
