	...
	return []ToolDefinition{
			...
			InputSchema: obj(
				...
				propObject(
					...
					obj(
						...
						prop("test_output", "string", "Trimmed test output summary (optional). Keep this short (key lines only); do NOT paste full logs."),
				...
				propObject(
					"test_evidence",
					"Lead-provided test evidence (required).",
					obj(
						prop("script_path", "string", "Path to test script (e.g. test-issue-xxx.sh)."),
						prop("script_cmd", "string", "Exact command used to run the test script (required)."),
						prop("script_passed", "boolean", "Whether the script run passed (required)."),
						prop("script_result", "string", "Script output summary (required). Keep this short (key lines only); do NOT paste full logs."),
						prop("doc_path", "string", "Path to test doc (e.g. test-issue-xxx.md)."),
						prop("doc_commands", "array", "Commands extracted from test doc (required)."),
						propArrayOfObject(
							"doc_results",
							"Execution results for doc_commands (required; must align by index).",
							obj(
								prop("command", "string", "Command string."),
								prop("passed", "boolean", "Whether the command passed."),
								prop("exit_code", "integer", "Process exit code."),
								prop("output", "string", "Command output summary. Keep this short (key lines only); do NOT paste full logs."),
								required("command", "passed", "exit_code", "output"),
							),
						),
						prop("doc_passed", "boolean", "Whether the full doc command set passed (required)."),
						required("script_path", "script_cmd", "script_passed", "script_result", "doc_path", "doc_commands", "doc_results", "doc_passed"),
					),
				),
				...
				required("session_id", "issue_id", "summary", "artifacts", "test_evidence"),
			),
		},
		{
			Name:        "claimDelivery",
		...
		},
		{
			...
			InputSchema: obj(
				...
				propObject(
					"verification",
					"Acceptor verification results (required).",
					obj(
						prop("script_passed", "boolean", "Whether acceptor script run passed (required)."),
						prop("script_result", "string", "Acceptor script output summary (required). Keep this short (key lines only); do NOT paste full logs."),
						prop("doc_passed", "boolean", "Whether acceptor doc command set passed (required)."),
						propArrayOfObject(
							"doc_results",
							"Execution results for delivery test_evidence.doc_commands (required; must align by index).",
							obj(
								prop("command", "string", "Command string."),
								prop("passed", "boolean", "Whether the command passed."),
								prop("exit_code", "integer", "Process exit code."),
...
func allToolsForRole(role string) []ToolDefinition {
	base := allTools()
	// Order matters:
	// 1) role-specific required fields (worker_id)
	// 2) role code
...
func toolAllowSetForRole(role string) map[string]bool {
	...
	case "worker":
		...
		allowed["getIssue"] = true
		allowed["getIssueTask"] = true
		allowed["extendIssueTaskLease"] = true
